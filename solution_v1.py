...
def summary_features(arr):
    ...
    cols += [
        ...
        (np.abs(f - f.mean()) > 0.05).mean(1), (f < f.mean()).mean(1),
...
for seed in SEEDS:
    ...
    for tr_idx, va_idx in splitter.split(train_seq, strata, groups):
        ...
        gr.fit(train_aux[tr_idx], y_route[tr_idx], sample_weight=sample_w[tr_idx] * cw_route[y_route[tr_idx]])
        gu = HistGradientBoostingClassifier(
            max_iter=250, learning_rate=0.06, max_leaf_nodes=15,
            l2_regularization=1.0, random_state=seed,
        )
        gu.fit(train_aux[tr_idx], y_urg[tr_idx], sample_weight=sample_w[tr_idx] * cw_urg[y_urg[tr_idx]])

        vr = BLEND * vr + (1 - BLEND) * gr.predict_proba(train_aux[va_idx])
        vu = BLEND * vu + (1 - BLEND) * gu.predict_proba(train_aux[va_idx])
        pr_t = BLEND * pr_t + (1 - BLEND) * gr.predict_proba(test_aux)
        pu_t = BLEND * pu_t + (1 - BLEND) * gu.predict_proba(test_aux)
