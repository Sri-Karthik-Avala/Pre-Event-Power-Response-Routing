...
def prior_offsets(prob, target_rate, weights):
    logp = np.log(np.clip(prob, 1e-9, None))
    bias = np.zeros(prob.shape[1])
    for _ in range(CAL_STEPS):
        pick = (logp + bias).argmax(1)
        rate = np.array([weights[pick == c].sum() for c in range(prob.shape[1])]) / weights.sum()
        bias += CAL_LR * (target_rate - rate)
    return bias
...
strata = y_route * len(urgencies) + y_urg
n_tr = len(train)
n_te = len(test)

oof_r = np.zeros((n_tr, len(routes)))
oof_u = np.zeros((n_tr, len(urgencies)))
test_r = np.zeros((n_te, len(routes)))
test_u = np.zeros((n_te, len(urgencies)))
n_members = len(SEEDS)

for seed in SEEDS:
    splitter = StratifiedGroupKFold(n_splits=FOLDS, shuffle=True, random_state=seed)
    for tr_idx, va_idx in splitter.split(train_seq, strata, groups):
        wr = sample_w[tr_idx] * cw_route[y_route[tr_idx]]
        wu = sample_w[tr_idx] * cw_urg[y_urg[tr_idx]]

        net = fit_net(train_seq, train_aux, y_route, y_urg, sample_w, cw_route, cw_urg, tr_idx, seed)
        vr, vu = net_predict(net, train_seq, train_aux, va_idx)
        pr_t, pu_t = net_predict(net, test_seq, test_aux, np.arange(n_te))

        er = ExtraTreesClassifier(600, min_samples_leaf=5, random_state=seed, n_jobs=4)
        er.fit(train_aux[tr_idx], y_route[tr_idx], sample_weight=wr)
        eu = ExtraTreesClassifier(600, min_samples_leaf=5, random_state=seed, n_jobs=4)
        eu.fit(train_aux[tr_idx], y_urg[tr_idx], sample_weight=wu)

        gr = HistGradientBoostingClassifier(
            max_iter=150, learning_rate=0.06, max_leaf_nodes=7,
            l2_regularization=5.0, random_state=seed,
        )
        gr.fit(train_aux[tr_idx], y_route[tr_idx], sample_weight=wr)
        gu = HistGradientBoostingClassifier(
            max_iter=150, learning_rate=0.06, max_leaf_nodes=7,
            l2_regularization=5.0, random_state=seed,
        )
        gu.fit(train_aux[tr_idx], y_urg[tr_idx], sample_weight=wu)

        vr = W_NET * vr + W_ET * er.predict_proba(train_aux[va_idx]) + W_GB * gr.predict_proba(train_aux[va_idx])
        vu = W_NET * vu + W_ET * eu.predict_proba(train_aux[va_idx]) + W_GB * gu.predict_proba(train_aux[va_idx])
        pr_t = W_NET * pr_t + W_ET * er.predict_proba(test_aux) + W_GB * gr.predict_proba(test_aux)
        pu_t = W_NET * pu_t + W_ET * eu.predict_proba(test_aux) + W_GB * gu.predict_proba(test_aux)

        oof_r[va_idx] += vr / n_members
        oof_u[va_idx] += vu / n_members
        test_r += pr_t / (FOLDS * n_members)
        test_u += pu_t / (FOLDS * n_members)

bias_r = prior_offsets(oof_r, route_prior, event_w)
bias_u = prior_offsets(oof_u, urg_prior, event_w)

oof_route_pick = (np.log(np.clip(oof_r, 1e-9, None)) + bias_r).argmax(1)
oof_urg_pick = (np.log(np.clip(oof_u, 1e-9, None)) + bias_u).argmax(1)
print("oof route agreement", float((oof_route_pick == y_route).mean()))
print("oof urgency agreement", float((oof_urg_pick == y_urg).mean()))

pick_r = (np.log(np.clip(test_r, 1e-9, None)) + bias_r).argmax(1)
pick_u = (np.log(np.clip(test_u, 1e-9, None)) + bias_u).argmax(1)
